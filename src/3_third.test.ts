...
class AdditionTransformation implements PaymentDateTransformation {
  #daysToAdd: number;
  constructor(daysToAdd: number) {
    this.#daysToAdd = daysToAdd;
  }
  transform(paymentDate: Date): Date {
    return new Date(
      paymentDate.getTime() + this.#daysToAdd * 24 * 60 * 60 * 1000
    );
  }
}

class MultiplicationTransformation implements PaymentDateTransformation {
  #multiplier: number;
  constructor(multiplier: number) {
    this.#multiplier = multiplier;
  }
  transform(paymentDate: Date): Date {
    let timeToPay = paymentDate.getTime() - Date.now();
    return new Date(Date.now() + timeToPay * this.#multiplier);
  }
}

class PaymentDateTransformationComposite implements PaymentDateTransformation {
  #transformations: PaymentDateTransformation[];
  constructor(transformations: PaymentDateTransformation[]) {
    this.#transformations = transformations;
  }
  transform(paymentDate: Date): Date {
    return this.#transformations.reduce(
      (acc, transformation) => transformation.transform(acc),
      paymentDate
    );
  }
}

class PaymentDateTransformationFactory {
  static create(
    supplierName: string,
    balance: number
  ): PaymentDateTransformation {
    const transformations: PaymentDateTransformation[] = [];
    if (supplierName.startsWith("Auto")) {
      transformations.push(new AdditionTransformation(5));
    }
    if (balance < 0) {
      transformations.push(new MultiplicationTransformation(2));
    }
    return new PaymentDateTransformationComposite(transformations);
  }
}

interface PaymentDateTransformation {
  transform(paymentDate: Date): Date;
}
...
class InvoiceService {
  ...
  payInvoice(
    ...
    const payment = new Payment(
      ...
      PaymentDateTransformationFactory.create(invoice.supplierName, CURRENT_BALANCE).transform(invoice.paymentDate),
      invoice.id
    );
